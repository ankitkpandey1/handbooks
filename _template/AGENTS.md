...
**Tier B.** Edition 0.1.0. See [`book.json`](book.json) for the machine-readable contract and
the repo root [`AGENTS.md`](../../AGENTS.md) for repo-wide rules.

Canonical source: `source/__SLUG__.md`
...
```bash
scripts/verify-book.sh __SLUG__            # structural lint (no toolchain needed)
scripts/build-book.sh __SLUG__ html md     # fast preview formats
scripts/build-book.sh __SLUG__             # everything in book.json build.formats
```

## What Tier B means here

Structural lint only: metadata validity, front matter, heading structure, licence
consistency, fence balance, dead internal links. There is **no** verifier suite binding the
